...
class Program
{
	static void Main(string[] args)
	{
		...
		// Fetch the data from MongoDB as SampleData
		var fetchedSampleData = collection.Find(new BsonDocument()).FirstOrDefault();

		// Assuming you have only one document, use the fetchedSampleData directly
		var xArray = fetchedSampleData.X.Select(value => (float)value).ToArray();
		var yArray = fetchedSampleData.Y.Select(value => (float)value).ToArray();
	...
	{
	 	public ObjectId Id { get; set; } // This corresponds to the MongoDB _id field
